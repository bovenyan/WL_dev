...
class req_handler(object):
    def __init__(self, config, dev_type):
        self.dbi = db_api(config, dev_type)
        self.config = config
        self.manage_timeout = None
        self.operate_sleep = int(self.config.get('opconfig', 'operateSL'))
        self.maxMgmtRenew = int(self.config.get('opconfig', 'maxMgmtRenew'))
    ...
    def reply_device_status(self, dev_id):
        ...
        """
        self.record = self.dbi.device_get_mode(dev_id)

        if not (isinstance(self.record, dict)) or not bool(self.record):
            abort(400)

        reply = {"mode": "operation", "options": {}, "reason": None}

        last_updated = self.record["last_operation"]
        self.manage_flags = self.record["manage_flags"]
        ...
        # management options
        self.op_codes = self.record["op_codes"]
        ...
        self.dbi.device_reset_usr(dev_id)
        """
        return None  # failed to find a match
    ...
    def handle_usr_enable_mgmt(self, dev_id, content):
        if bool(content):
            ...
            if not result[1]:   # meed wait
                max_wait = timedelta(0, self.operate_sleep, 0)
                max_wait = max_wait.seconds
                have_wait = datetime.now() - self.dbi.get_lastseen(dev_id)
                have_wait = have_wait.total_seconds()

                if (max_wait < have_wait):
                    miss = have_wait/max_wait
                    wait = max_wait - have_wait % max_wait
                    return {"success": True, "wait": wait, "miss": miss}
                else:
                    wait = max_wait - have_wait
    ...
    def handle_usr_mgmt_renew(self, dev_id, time):
        if self.handle_usr_check_mode(dev_id):
            if time <= 0 or time > self.maxMgmtRenew:
