...
class servo_sig(object):
	def __init__(self, conf_path):
		...
		lock = Lock()
		self.servo_x = servo(config, 0, self.blaster, lock, self.des_queue_x, self.cur_queue_x)
		self.servo_y = servo(config, 1, self.blaster, lock, self.des_queue_y, self.cur_queue_y)
	...
	def signal_channel(self):
		...
		while True:
			...
			print ans
			if ('manage' in ans) and (ans['manage']):  # management mode 
				...
				if ('servo_active' in ans and ans['servo_active']):  # move servo
				...
				# TODO take video and upload
				if ('video' in ans and ans['video']):
					pass
				...
				# TODO check conn
				resp = requests.post(dev_url, data=json.dumps(response), headers=headers)
			else: 
				sleep(self.op_sleep)
...
class servo(object):
	def __init__(self, config, sID, blaster, lock, des_queue, cur_queue):
		...
		self.servo_str = "P1-"+config.get("servoConfig", "servo"+str(sID)+"Pin")+"="
		self.sID = sID
		...
		# init 
		lock.acquire()
		self.blaster.write(self.servo_str + str(90 * self.ratio + self.servo_voltage[0])+'\n')
		self.blaster.flush()	
		lock.release()
	...
	def move(self):
		...
		while True:
			...
			if (self.des_queue.empty()):
				...
				if (des_pos == cur_pos or des_pos > 90 or des_pos < -90):  # posistion range [-90, 90]
					pass
				else:	
					volt = float(des_pos + 90) * self.ratio + self.servo_voltage[0]
...
if __name__ == "__main__":
	ss = servo_sig("/home/pi/wikkit/signal/config.ini")
